...
import { FiCpu, FiTrash2, FiMenu } from "react-icons/fi"; // Added FiMenu for mobile drawer action
    ...
    <div className="flex-1 h-full flex flex-col bg-slate-50 dark:bg-[#070a12]/30 relative">
      ...
      {!selectedConversation ? (
        <div className="flex-1 flex flex-col items-center justify-center relative z-10 p-8 text-center animate-pulse">
          {/* Mobile Menu Trigger for clean onboarding entry state */}
          <button 
            onClick={() => setIsMobileOpen(true)}
            className="md:hidden absolute top-5 left-5 p-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300"
          >
        ...
        <>
          ...
          <div className="p-5 bg-white dark:bg-[#0d1321]/40 border-b border-slate-200 dark:border-slate-800/60 backdrop-blur-md flex items-center gap-3 relative z-10">
            {/* Mobile Menu Drawer Toggle anchor button link */}
            <button
              onClick={() => setIsMobileOpen(true)}
              className="md:hidden p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-900 text-slate-600 dark:text-slate-400 focus:outline-none"
            >
              <FiMenu className="w-5 h-5" />
            </button>
              ...
              return (
                ...
                >
                  ...
                  <div className={`flex items-center gap-2 max-w-[85%] md:max-w-md ${isMe ? "flex-row-reverse" : "flex-row"}`}>
                    ...
                    <div 
                      className={`p-3.5 rounded-2xl text-sm leading-relaxed shadow-xs border transition-all duration-300 ${
                        isMe 
                    ...
                    {isMe && (
                      <button
                        ...
                        className="opacity-0 group-hover/msg:opacity-100 p-2 text-slate-400 hover:text-red-500 dark:text-slate-500 dark:hover:text-red-400 bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-lg shadow-sm transition-all duration-200 cursor-pointer focus:outline-none shrink-0"
                      >
